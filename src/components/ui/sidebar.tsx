...
function Sidebar({
  ...
  if (isMobile) {
    /**
     * Inline slide-over — **no Dialog/Sheet portal**.
     * Base UI `Sheet` portals to `document.body`; toggling `isMobile` swapped entire subtree and
     * triggered `removeChild` / NotFoundError under React 19 + Turbopack during login redirects.
     */
    return (
      <>
        <div
          ...
          className={cn(
            "supports-backdrop-filter:backdrop-blur-xs fixed inset-0 z-40 bg-black/10 transition-opacity duration-150",
        ...
        <div
          ...
          className={cn(
            "w-(--sidebar-width) bg-sidebar text-sidebar-foreground fixed z-50 flex h-svh flex-col p-0 shadow-lg transition-transform duration-200 ease-out",
            side === "left"
              ? "border-sidebar-border inset-y-0 left-0 border-r"
              : "border-sidebar-border inset-y-0 right-0 border-l",
            openMobile
              ? "translate-x-0"
              : side === "left"
                ? "-translate-x-full"
